...
/* MASTER HEADER */
#master-hdr{background:var(--djp-dark);border-bottom:3px solid var(--djp-gold);flex-shrink:0;z-index:400;box-shadow:0 2px 8px rgba(0,0,0,.3)}
#hdr-top{padding:8px 18px;display:flex;align-items:center;gap:12px}
...
#tab-bar{display:flex;padding:0 8px;gap:2px;background:rgba(0,0,0,.2)}
...
/* DASHBOARD STYLES */
#dash-ctrl{background:var(--djp-dark);padding:6px 18px;display:flex;align-items:center;gap:14px;flex-shrink:0}
...
#kpistrip{background:linear-gradient(135deg,#0d2d5a 0%,#0a2346 100%);border-bottom:2px solid var(--djp-gold);padding:6px 18px;display:flex;gap:7px;flex-shrink:0}
...
#dash-body{flex:1;display:flex;flex-direction:column;overflow:hidden;padding:10px 16px 0}
...
#scrollarea{flex:1;display:flex;gap:6px;overflow:hidden;min-height:0;padding-bottom:10px}
...
#tblbox{flex:1;display:flex;flex-direction:column;border:1px solid var(--bd);border-radius:8px;background:var(--sr);overflow:hidden;min-width:0;box-shadow:0 2px 8px rgba(0,0,0,.06)}
#tblscroll{flex:1;overflow:auto;min-height:0}
#tblscroll::-webkit-scrollbar{width:5px;height:5px}
#tblscroll::-webkit-scrollbar-track{background:var(--s2)}
#tblscroll::-webkit-scrollbar-thumb{background:var(--bds);border-radius:3px}
...
/* PETA STYLES */
#peta-wrap{display:flex;flex-direction:column;height:100%;overflow:hidden;font-family:'Plus Jakarta Sans',sans-serif;background:#eef2f7}
#peta-sbar{background:#f6f9fc;border-bottom:1px solid #dae1ea;padding:4px 18px;display:flex;overflow-x:auto;flex-shrink:0}
...
#peta-crow{display:flex;flex:1;overflow:hidden;position:relative}
#sidebar{width:280px;min-width:280px;background:#fff;border-right:1px solid #dae1ea;display:flex;flex-direction:column;overflow:hidden;transition:width .3s ease,min-width .3s ease;flex-shrink:0;box-shadow:2px 0 8px rgba(0,0,0,.04);z-index:10}
#stog{position:absolute;left:280px;top:50%;transform:translateY(-50%);z-index:1001;width:20px;height:48px;background:#fff;border:1px solid #dae1ea;border-left:none;border-radius:0 6px 6px 0;cursor:pointer;display:flex;align-items:center;justify-content:center;font-size:12px;color:#8fa3b8;transition:left .3s ease,background .15s;box-shadow:2px 0 6px rgba(0,0,0,.06);font-family:inherit}
#stog:hover{background:#e8f0fe;color:#1a56db}
...
#klu-count{font-size:10px;font-weight:700;color:#1a56db}
#klu-clear{display:none;align-items:center;gap:3px;font-size:10px;font-weight:700;color:#dc2626;cursor:pointer;background:#fef2f2;border:1px solid #fecaca;border-radius:5px;padding:2px 7px;white-space:nowrap;font-family:inherit}
...
#leg{display:none;position:absolute;bottom:20px;right:14px;z-index:1000;background:#fff;border:1px solid #dae1ea;border-radius:9px;padding:12px 14px;box-shadow:0 2px 10px rgba(0,0,0,.08);min-width:155px}
#leg h4{font-size:10px;font-weight:800;color:#8fa3b8;text-transform:uppercase;letter-spacing:.6px;margin-bottom:7px}
...
<div id="master-hdr">
  <div id="hdr-top">
    <span class="hdr-badge">LHA</span>
    <div class="hdr-titles">
      <h1>{{TITLE}}</h1>
      <p>Direktorat Jenderal Pajak &nbsp;&middot;&nbsp; &#x1F512; RAHASIA</p>
    </div>
  </div>
  <div id="tab-bar">
    <button class="tab-btn active" id="tab-btn-dash" onclick="switchTab('dash')">&#x1F4CA;&nbsp; Dashboard Monitoring LHA</button>
    <button class="tab-btn" id="tab-btn-peta" onclick="switchTab('peta')">&#x1F5FA;&nbsp; Peta Sebaran LHA</button>
  </div>
</div>
<div id="content-area">
  <!-- PANEL DASHBOARD -->
  <div class="tab-panel active" id="panel-dash">
    <div id="dash-ctrl">
      <div class="pnrb-wrap">
        ...
        <label class="pnrb-item"><input type="checkbox" id="ckKP" checked onchange="onFilter()"><label for="ckKP">Kantor Pusat</label></label>
        <label class="pnrb-item"><input type="checkbox" id="ckKW" checked onchange="onFilter()"><label for="ckKW">Kanwil</label></label>
      </div>
      <div class="divider"></div>
      <span class="ctrl-lbl">Tahun Pajak:</span>
      <select class="ysel" id="ysel" onchange="onFilter()">{{YEAR_OPTS}}</select>
      <span class="yb" id="ybadge">ALL</span>
    </div>
    <div id="kpistrip"></div>
    <div id="dash-body">
      <div class="ttb">
        <div><div class="ttl">Data LHA per Kanwil &amp; KPP</div><div class="tmt" id="tmeta">Klik Kanwil/KPP untuk expand detail</div></div>
        <div class="bg">
      ...
      <div id="scrollarea">
        <div class="vsl-wrap"><div class="vsl-lbl">SCROLL</div>
          <input type="range" class="vsl" id="vsl" min="0" max="10000" value="0" oninput="syncV(this.value)">
        </div>
        <div id="tblbox">
          <div id="tblscroll">
            <table>
          ...
          <div class="hsl-bar">
            ...
            <input type="range" class="hsl" id="hsl" min="0" max="10000" value="0" oninput="syncH(this.value)">
          </div>
        </div>
      </div>
    </div>
  ...
  <!-- PANEL PETA -->
  <div class="tab-panel" id="panel-peta">
    <div id="peta-wrap">
      <div id="peta-sbar">
        <div class="sc"><span class="lb">Total LHA</span><span class="vl" id="s-tot">&mdash;</span></div>
        <div class="sc"><span class="lb">Total Potensi LHA</span><span class="vl" id="s-pot">&mdash;</span></div>
        <div class="sc"><span class="lb">Total Pembayaran</span><span class="vl" id="s-bay">&mdash;</span></div>
        <div class="sc"><span class="lb">SP2DK Terbit</span><span class="vl" id="s-sp2dk">&mdash;</span></div>
        <div class="sc"><span class="lb">KPP Terdampak</span><span class="vl" id="s-kpp">&mdash;</span></div>
        <div class="sc"><span class="lb">Tampilan</span><span class="vl" id="s-mod">Point KPP</span></div>
      </div>
      <div id="peta-crow">
        <div id="sidebar">
          <div class="sb-body">
            <div class="sb-sec">
              ...
              <div class="tg2">
                <button class="tb2 on" id="tb-pt" onclick="setMode('point')">&#x1F4CD; Point KPP</button>
                <button class="tb2" id="tb-ar" onclick="setMode('area')">&#x1F30F; Area Provinsi</button>
              </div>
            </div>
            <div class="sb-sec">
              <div class="sb-sec-title">&#x1F3A8; Metrik Warna</div>
              <div class="tg2">
                <button class="tb2 on" id="m-seb" onclick="setMetric('sebaran')">Sebaran LHA</button>
                <button class="tb2" id="m-pot" onclick="setMetric('potensi')">Potensi LHA</button>
              </div>
            </div>
            <div class="sb-sec">
            ...
            <div class="sb-sec">
              ...
              <div id="kchips">{{KLU_CHIPS}}</div>
            </div>
          </div>
        </div>
        <button id="stog" onclick="toggleSidebar()" title="Sembunyikan panel">&#x276E;</button>
        <div id="marea"><div id="map"></div><div id="leg"></div></div>
      </div>
    </div>
  </div>
</div>
...
function getEff(e){var kp=document.getElementById('ckKP').checked,kw=document.getElementById('ckKW').checked;if(kp&&kw)return e.tt;if(kp)return e.kp;if(kw)return e.kw;return zeroAgg(e.tt);}
function getNatEff(n){var kp=document.getElementById('ckKP').checked,kw=document.getElementById('ckKW').checked;if(kp&&kw)return n.all;if(kp)return n.kp;if(kw)return n.kw;return zeroAgg(n.all);}
function rKpi(){var n=getNatEff(_D.nat[_cy]||_D.nat['ALL']);var cards=[{l:'Total LHA',v:fI(n.n),s:'Seluruh nasional',c:''},{l:'Potensi LHA',v:fR(n.pot),s:'Nilai potensi',c:''},{l:'Terbit SP2DK',v:fI(n.sp_t),s:'Blm: '+fI(n.sp_b),c:'gn'},{l:'Pot. Awal SP2DK',v:fR(n.sp_t_v),s:'Terbit',c:'gn'},{l:'Terbit LHP2DK',v:fI(n.lhp_t),s:'Blm: '+fI(n.lhp_b),c:'pu'},{l:'Pot. LHP2DK',v:fR(n.lhp_t_v),s:'Terbit',c:'pu'},{l:'Selesai',v:fI(n.sel),s:'Real+TA+PPS+Close',c:'am'},{l:'Nilai Realisasi',v:fR(n.sel_rv),s:'Pembayaran',c:'am'},{l:'Open',v:fI(n.opn),s:'Pot: '+fR(n.opn_pot),c:'rd'}];document.getElementById('kpistrip').innerHTML=cards.map(function(c){return'<div class="kc '+c.c+'"><div class="kl">'+c.l+'</div><div class="kv">'+c.v+'</div><div class="ks2">'+c.s+'</div></div>';}).join('');}
...
function onFilter(){var kp=document.getElementById('ckKP'),kw=document.getElementById('ckKW');if(!kp.checked&&!kw.checked){kp.checked=true;return;}_cy=document.getElementById('ysel').value;document.getElementById('ybadge').textContent=_cy;_ek={};_ep={};rKpi();rTable(_cy);}
function iSlider(){var ts=document.getElementById('tblscroll'),h=document.getElementById('hsl'),v=document.getElementById('vsl');setTimeout(function(){h.max=Math.max(1,ts.scrollWidth-ts.clientWidth);v.max=Math.max(1,ts.scrollHeight-ts.clientHeight);h.value=ts.scrollLeft;v.value=ts.scrollTop;},50);}
function syncH(val){document.getElementById('tblscroll').scrollLeft=parseInt(val);}
function syncV(val){document.getElementById('tblscroll').scrollTop=parseInt(val);}
document.getElementById('tblscroll').addEventListener('scroll',function(){var h=document.getElementById('hsl'),v=document.getElementById('vsl');h.max=Math.max(1,this.scrollWidth-this.clientWidth);v.max=Math.max(1,this.scrollHeight-this.clientHeight);h.value=this.scrollLeft;v.value=this.scrollTop;});
...
def build_html(payload, years):
    """Embed payload into HTML template."""
    json_str = json.dumps(payload, allow_nan=False, separators=(',', ':'))
    b64 = base64.b64encode(json_str.encode()).decode()
    chunks = [b64[i:i+76] for i in range(0, len(b64), 76)]
    data_js = '+\n'.join([f'"{c}"' for c in chunks])

    # Build year options
    year_opts = '<option value="ALL">Semua Tahun</option>\n'
    for y in sorted([yr for yr in years if yr != 'ALL']):
        year_opts += f'<option value="{y}">{y}</option>\n'

    html = HTML_BEFORE.replace('YEAR_OPTIONS', year_opts)
    html += data_js
    html += HTML_AFTER
    return html
...
# BUILD COMBINED HTML
# =============================================================
def build_combined_html(dash_payload, dash_years,
                        peta_b64, pub_vals, jenis_vals, klu_vals,
                        title="Dashboard LHA DJP", log=None):
    """Output: satu HTML dengan tab Dashboard + Peta."""
    if log: log("Menyusun HTML gabungan...")

    # Dashboard data
    json_str = json.dumps(dash_payload, allow_nan=False, separators=(',',':'))
    d_b64    = base64.b64encode(json_str.encode()).decode()
    chunks   = [d_b64[i:i+76] for i in range(0, len(d_b64), 76)]
    data_js  = '+\n'.join([f'"{c}"' for c in chunks])

    year_opts = '<option value="ALL">Semua Tahun</option>\n'
    for y in sorted([yr for yr in dash_years if yr != "ALL"]):
        year_opts += f'<option value="{y}">{y}</option>\n'

    # Peta chips
    pub_chips = ""
    for pub in pub_vals:
        pub_chips += _chip(pub, PUB_COLORS.get(pub,"#555"), "chip-pub", "togglePub",
                           PUB_LABELS.get(pub, pub))
    jen_chips = ""
    for i, jen in enumerate(jenis_vals):
        jen_chips += _chip(jen, JENIS_COLORS[i % len(JENIS_COLORS)], "chip-jen", "toggleJenis")
    klu_upper = {k.upper() for k in klu_vals}
    ordered   = [k for k in KLU_ORDER if k.upper() in klu_upper]
    extra     = [k for k in klu_vals if k.upper() not in {x.upper() for x in KLU_ORDER}]
    klu_chips = ""
    for i, klu in enumerate(ordered + extra):
        klu_chips += _chip(klu, KLU_COLORS[i % len(KLU_COLORS)], "chip-klu", "toggleKLU")

    html = _HTML_TEMPLATE
    html = html.replace("{{TITLE}}",           title)
    html = html.replace("{{YEAR_OPTS}}",       year_opts)
    html = html.replace("{{PUB_CHIPS}}",       pub_chips)
    html = html.replace("{{JEN_CHIPS}}",       jen_chips)
    html = html.replace("{{KLU_CHIPS}}",       klu_chips)
    html = html.replace("{{DASH_DATA_JS}}",    data_js)
    html = html.replace("{{PETA_PAYLOAD_B64}}",peta_b64)
    html = html.replace("{{GEO_B64}}",         GEO_B64)
    html = html.replace("{{KREF_B64}}",        KREF_B64)
    html = html.replace("{{KWP_B64}}",         KWP_B64)
    html = html.replace("{{JS_B64}}",          JS_B64)

    if log: log(f"HTML selesai: {len(html)//1024} KB")
    return html

# ── COLUMN MAPPING DIALOG ─────────────────────────────────────
class MappingDialog(tk.Toplevel):
    def __init__(self, parent, available_cols, missing_cols):
        super().__init__(parent)
        self.title("Mapping Kolom")
        self.resizable(False, False)
        self.grab_set()
        self.result = {}
        self.vars = {}

        tk.Label(self, text="Beberapa kolom tidak ditemukan.\nPilih kolom yang sesuai dari file Anda:",
                 justify='left', font=('Segoe UI', 10), pady=10, padx=15).pack(anchor='w')

        frame = ttk.Frame(self, padding=15)
        frame.pack(fill='both', expand=True)

        for i, (std_col, description) in enumerate(missing_cols.items()):
            ttk.Label(frame, text=f"{std_col}", font=('Segoe UI', 9, 'bold')).grid(
                row=i, column=0, sticky='w', pady=4, padx=(0, 10))
            ttk.Label(frame, text=f"({description})", font=('Segoe UI', 8),
                      foreground='gray').grid(row=i, column=1, sticky='w', pady=4, padx=(0, 20))
            var = tk.StringVar(value='-- Pilih Kolom --')
            self.vars[std_col] = var
            cb = ttk.Combobox(frame, textvariable=var, values=['-- Lewati --'] + available_cols,
                              width=35, state='readonly')
            cb.grid(row=i, column=2, sticky='w', pady=4)

        btn_frame = ttk.Frame(self, padding=(15, 5, 15, 15))
        btn_frame.pack(fill='x')
        ttk.Button(btn_frame, text="Terapkan", command=self._apply).pack(side='right', padx=5)
        ttk.Button(btn_frame, text="Batal", command=self.destroy).pack(side='right')

        self.geometry(f"+{parent.winfo_x()+50}+{parent.winfo_y()+50}")

    def _apply(self):
        for std_col, var in self.vars.items():
            val = var.get()
            if val not in ('-- Pilih Kolom --', '-- Lewati --'):
                self.result[std_col] = val
        self.destroy()
...
class GeneratorApp(tk.Tk):
    def __init__(self):
        super().__init__()
        self.title("Generator LHA DJP - Gabungan")
        self.geometry("660x580")
        self.minsize(600,500)
        self.configure(bg='#0a2346')
        self.resizable(True, True)

        self.lha_path  = tk.StringVar()
        self.sheet_var = tk.StringVar(value='Gabungan')
        self.title_var = tk.StringVar(value='Dashboard Laporan Hasil Analisis DJP')
        self.col_map   = {k: k for k in DASH_REQUIRED_COLS}
        self._out_path = None

        self._build_ui()
        if not _ASSETS_LOADED:
            self._log("PERINGATAN: _peta_assets.py tidak ditemukan.\n"
                      "Fitur PETA di output HTML tidak akan berfungsi.\n"
                      "Pastikan _peta_assets.py ada di folder yang sama.")

    # ── UI ───────────────────────────────────────────────────────
    def _build_ui(self):
        # Header
        hdr = tk.Frame(self, bg='#0a2346', pady=14)
        hdr.pack(fill='x')
        inn = tk.Frame(hdr, bg='#0a2346'); inn.pack()
        tk.Label(inn, text="LHA", bg='#c8a84b', fg='#0a2346',
                 font=('Segoe UI',10,'bold'), padx=10, pady=2).pack(side='left', padx=(0,10))
        txt = tk.Frame(inn, bg='#0a2346'); txt.pack(side='left')
        tk.Label(txt, text="Generator HTML LHA DJP - Gabungan",
                 font=('Segoe UI',13,'bold'), fg='#e8c96a', bg='#0a2346').pack(anchor='w')
        tk.Label(txt, text="Direktorat Jenderal Pajak  |  Dashboard + Peta dalam Satu File",
                 font=('Segoe UI',9), fg='#c8a84b', bg='#0a2346').pack(anchor='w')
        tk.Frame(self, bg='#c8a84b', height=2).pack(fill='x')

        body = tk.Frame(self, bg='#f0f2f5', padx=24, pady=18)
        body.pack(fill='both', expand=True)

        def lbl(row, text):
            tk.Label(body, text=text, font=('Segoe UI',9),
                     bg='#f0f2f5').grid(row=row, column=0, sticky='w', pady=6, padx=(0,10))

        # File LHA
        lbl(0, "File Excel LHA:")
        ttk.Entry(body, textvariable=self.lha_path, width=42).grid(
            row=0, column=1, sticky='ew', pady=6, padx=(0,6))
        tk.Button(body, text="Pilih", command=self._browse_lha,
                  bg='#e2e8f0', relief='flat', padx=8, cursor='hand2').grid(row=0, column=2, sticky='w')

        # Sheet
        lbl(1, "Sheet:")
        sf = tk.Frame(body, bg='#f0f2f5'); sf.grid(row=1, column=1, sticky='w', pady=4)
        self.sheet_cb = ttk.Combobox(sf, textvariable=self.sheet_var, state='disabled', width=22)
        self.sheet_cb.pack(side='left')
        self._sheet_info = tk.Label(sf, text="", bg='#f0f2f5', fg='#6b7280',
                                    font=('Segoe UI',8,'italic')); self._sheet_info.pack(side='left',padx=(8,0))

        # Judul
        lbl(2, "Judul HTML:")
        ttk.Entry(body, textvariable=self.title_var, width=42).grid(
            row=2, column=1, columnspan=2, sticky='ew', pady=6)

        # Output path
        lbl(3, "Simpan ke:")
        self._out_lbl = tk.Label(body, text="Belum dipilih", bg='#ffffff', fg='#9ca3af',
                                 font=('Segoe UI',9), relief='solid', bd=1,
                                 anchor='w', padx=8, pady=5)
        self._out_lbl.grid(row=3, column=1, sticky='ew', pady=6, padx=(0,6))
        tk.Button(body, text="Pilih", command=self._browse_out,
                  bg='#e2e8f0', relief='flat', padx=8, cursor='hand2').grid(row=3, column=2, sticky='w')

        body.columnconfigure(1, weight=1)

        # Log
        tk.Label(body, text="Status:", font=('Segoe UI',9),
                 bg='#f0f2f5').grid(row=4, column=0, sticky='nw', pady=(14,4))
        self._log_txt = tk.Text(body, height=6, font=('Courier New',8),
                                state='disabled', bg='#1c2b3a', fg='#e2e8f0',
                                relief='flat', bd=1, padx=6, pady=5)
        self._log_txt.grid(row=4, column=1, columnspan=2, sticky='ew', pady=(14,4))

        # Progress
        self._prog = ttk.Progressbar(body, length=420, mode='determinate')
        self._prog.grid(row=5, column=1, columnspan=2, sticky='ew', pady=(6,2))
        self._prog_lbl = tk.Label(body, text="", font=('Segoe UI',8),
                                  fg='#6b7280', bg='#f0f2f5')
        self._prog_lbl.grid(row=6, column=1, sticky='w')

        # Buttons
        bf = tk.Frame(body, bg='#f0f2f5'); bf.grid(row=7, column=0, columnspan=3, pady=(16,0))
        tk.Button(bf, text="  Cek & Mapping Kolom", command=self._check_columns,
                  font=('Segoe UI',10), bg='#1d4ed8', fg='white',
                  relief='flat', padx=14, pady=7, cursor='hand2').pack(side='left', padx=8)
        tk.Button(bf, text="  Generate HTML Gabungan", command=self._generate,
                  font=('Segoe UI',10,'bold'), bg='#0a2346', fg='#e8c96a',
                  relief='flat', padx=14, pady=7, cursor='hand2').pack(side='left', padx=8)

    # ── BROWSE ───────────────────────────────────────────────────
    def _browse_lha(self):
        p = filedialog.askopenfilename(
            title="Pilih File Excel LHA",
    ...
    def _load_sheets(self, path):
        try:
            ...
            preferred = ['Gabungan','Data LHA','Data','LHA','Sheet1']
            sel = next((p for p in preferred if p in sheets), sheets[0])
            self.sheet_var.set(sel)
            self._sheet_info.configure(text=f"{len(sheets)} sheet")
            self._log(f"File: {os.path.basename(path)}\nSheet: {sel}  (tersedia: {len(sheets)})")
        except Exception as e:
            self._log(f"Gagal baca sheet: {e}")

    def _browse_out(self):
        p = filedialog.asksaveasfilename(
            title="Simpan HTML Gabungan",
            defaultextension=".html",
            filetypes=[('HTML', '*.html'), ('All', '*.*')],
            initialfile="LHA_DJP_Gabungan.html")
        if p:
            self._out_path = p
            self._out_lbl.configure(text=os.path.basename(p), fg='#111827')

    # ── CHECK COLUMNS ────────────────────────────────────────────
    def _check_columns(self):
        if not self.lha_path.get():
            messagebox.showwarning("Perhatian", "Pilih file LHA terlebih dahulu."); return
        try:
            df_s = pd.read_excel(self.lha_path.get(), sheet_name=self.sheet_var.get(), nrows=3)
            avail = df_s.columns.tolist()
        except Exception as e:
            messagebox.showerror("Error", f"Gagal membaca file:\n{e}"); return

        self.col_map = {k: k for k in DASH_REQUIRED_COLS}
        missing = {k: v for k, v in DASH_REQUIRED_COLS.items() if k not in avail}
        lines = []
        for sc in DASH_REQUIRED_COLS:
            lines.append(f"  {'OK ' if sc in avail else '!! '} {sc}")
        self._log('\n'.join(lines))

        if missing:
            dlg = MappingDialog(self, avail, missing)
            dlg.wait_window()
            if dlg.result:
                self.col_map.update(dlg.result)
                self._log("Mapping:\n" + '\n'.join([f"  {k} <- '{v}'" for k,v in dlg.result.items()]))
        else:
            self._log("Semua kolom ditemukan! Siap diproses.")
    ...
    def _generate(self):
        ...
        if not self._out_path:
            messagebox.showwarning("Perhatian", "Pilih lokasi simpan HTML terlebih dahulu."); return
        threading.Thread(target=self._run, daemon=True).start()

    def _run(self):
        try:
            self._upd(0, "Membaca file LHA...")
            sheet  = self.sheet_var.get() or 'Gabungan'
            df_raw = pd.read_excel(self.lha_path.get(), sheet_name=sheet, dtype=str)

            # Konversi kolom numerik
            cu = {c.strip().upper(): c for c in df_raw.columns}
            for nc in ['POTENSI_LHA','POTENSI_AWAL','POTENSI_AKHIR','NILAI_PEMBAYARAN']:
                ac = cu.get(nc)
                if ac:
                    df_raw[ac] = pd.to_numeric(
                        df_raw[ac].str.replace(',','',regex=False), errors='coerce').fillna(0)

            self._log(f"Dibaca: {len(df_raw):,} baris  |  {len(df_raw.columns)} kolom")
            self._upd(8, f"File dibaca: {len(df_raw):,} baris")

            # ── Dashboard ─────────────────────────────────────────
            self._upd(10, "Memproses Dashboard...")
            df_map   = pd.DataFrame(KPP_MAPPING)
            rn_map   = {}
            for sc, ac in self.col_map.items():
                if ac != sc and ac in df_raw.columns:
                    rn_map[ac] = sc
            df_dash  = df_raw.rename(columns=rn_map).copy()
            for sc in DASH_REQUIRED_COLS:
                if sc not in df_dash.columns:
                    df_dash[sc] = None

            def dp_cb(pct, msg): self._upd(10 + int(pct * 0.40), msg)
            dash_payload, dash_years = dash_process_data(df_dash, df_map, self.col_map, dp_cb)
            self._log(f"Dashboard: {len(dash_years)-1} tahun pajak")
            self._upd(52, "Dashboard selesai")

            # ── Peta ──────────────────────────────────────────────
            if _ASSETS_LOADED:
                self._upd(54, "Memproses Peta...")
                col_map_peta, _ = validate_columns(df_raw, self._log)
                peta_b64, pub_v, jen_v, klu_v = peta_process_data(
                    df_raw.copy(), col_map_peta, self._log)
                self._log(f"Peta: {len(pub_v)} penerbit, {len(jen_v)} jenis")
                self._upd(82, "Peta selesai")
            else:
                self._log("PERINGATAN: Peta dilewati (_peta_assets.py tidak ada)")
                peta_b64, pub_v, jen_v, klu_v = "", [], [], []
                self._upd(82, "Peta dilewati")

            # ── Build HTML ────────────────────────────────────────
            self._upd(85, "Menyusun HTML gabungan...")
            html = build_combined_html(
                dash_payload, dash_years,
                peta_b64, pub_v, jen_v, klu_v,
                self.title_var.get(), self._log)

            self._upd(97, "Menyimpan...")
            with open(self._out_path, 'w', encoding='utf-8') as f:
                f.write(html)

            sz = os.path.getsize(self._out_path)
            self._upd(100, f"Selesai! {os.path.basename(self._out_path)} ({sz//1024} KB)")
            self.after(0, lambda: messagebox.showinfo("Berhasil!",
                f"File HTML berhasil dibuat!\n\n"
                f"Lokasi : {self._out_path}\n"
                f"Ukuran : {sz//1024} KB\n"
                f"Baris  : {len(df_raw):,}\n\n"
                f"Buka di Chrome/Edge.\n"
                f"Klik tab di atas untuk beralih Dashboard ↔ Peta."))

        except Exception as e:
            self._upd(0, f"ERROR: {e}")
            self.after(0, lambda: messagebox.showerror("Error", f"{e}\n\n{traceback.format_exc()}"))

    # ── HELPERS ──────────────────────────────────────────────────
    def _upd(self, val, msg):
        self.after(0, lambda: self._prog.configure(value=val))
    ...
    def _log_add(self, msg):
        self.after(0, lambda m=msg: self._do_log_add(m))

    def _do_log_add(self, msg):
        self._log_txt.configure(state='normal')
