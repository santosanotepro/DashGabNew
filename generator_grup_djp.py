# -*- coding: utf-8 -*-
"""
Generator Dashboard Grup Perusahaan — DJP
Dibuat untuk Python 3.11.9+
Menghasilkan dashboard HTML interaktif dari template Excel DJP.
"""
...
def generate_html(perusahaan, output_path, source_filename):
...
<title>Dashboard Grup Perusahaan — DJP</title>
<style>
...
<div class="topbar">
  <div class="topbar-brand">
    ...
    <div>
      <div class="topbar-title">Dashboard Grup Perusahaan</div>
      <div class="topbar-sub">Direktorat Jenderal Pajak — Analisa Data LHA Grup</div>
    </div>
  </div>
  <div class="topbar-meta">
    Generated: {now}<br>
    Sumber: {os.path.basename(source_filename)}
  </div>
</div>

<!-- TABS -->
<div class="tabs">
  <button class="tab-btn active" onclick="switchTab('dashboard',this)">📊 Dashboard &amp; Tabel</button>
  <button class="tab-btn" onclick="switchTab('network',this)">🕸 Peta Jaringan</button>
</div>
...
<div class="kpi-strip" id="kpi-strip">
  <div class="kpi-card dark">
    ...
    <div class="kpi-val" id="kpi-grup">{total_grup}</div>
    <div class="kpi-label">Total Grup</div>
    <div class="kpi-sub" id="kpi-grup-sub">&nbsp;</div>
  </div>
  <div class="kpi-card">
    <div class="kpi-icon">🏗</div>
    <div class="kpi-val" id="kpi-perus">{total_perusahaan}</div>
    <div class="kpi-label">Perusahaan</div>
    <div class="kpi-sub" id="kpi-perus-sub">&nbsp;</div>
  </div>
  <div class="kpi-card green">
    <div class="kpi-icon">✅</div>
    <div class="kpi-val" id="kpi-terbit">{total_lha_terbit}</div>
    <div class="kpi-label">LHA Diterbitkan</div>
    <div class="kpi-sub" id="kpi-terbit-sub">&nbsp;</div>
  </div>
  <div class="kpi-card red">
    <div class="kpi-icon">⏳</div>
    <div class="kpi-val" id="kpi-belum">{total_lha_belum}</div>
    <div class="kpi-label">Belum Terbit</div>
    <div class="kpi-sub" id="kpi-belum-sub">&nbsp;</div>
  </div>
  <div class="kpi-card">
    <div class="kpi-icon">💰</div>
    <div class="kpi-val" id="kpi-potensi">--</div>
    <div class="kpi-label">Total Potensi</div>
    <div class="kpi-sub" id="kpi-potensi-sub">(Rp000)</div>
  </div>
  <div class="kpi-card amber">
    <div class="kpi-icon">💵</div>
    <div class="kpi-val" id="kpi-realisasi">--</div>
    <div class="kpi-label">Realisasi</div>
    <div class="kpi-sub" id="kpi-eff-sub">Efektivitas: --</div>
  </div>
</div>
...
<div class="filter-bar-row">
  ...
  <div class="filter-item">
    ...
    <select id="sel-grup" onchange="filterByGrup()">
  ...
  <div class="filter-item">
    ...
    <select id="sel-status" onchange="filterByGrup()">
  ...
  <div class="filter-item">
    ...
    <select id="sel-tahun" onchange="filterByGrup()">
  ...
  <div class="filter-item" style="flex:2; min-width:200px;">
    ...
    <input type="text" id="search-box" placeholder="Nama perusahaan..." oninput="filterByGrup()">
  </div>

</div>

<!-- DISTRIBUSI LEVEL (full width, horizontal) -->
<div class="level-dist-full" id="level-dist-block">
  <div class="level-dist-title">📊 Distribusi Level Perusahaan</div>
  <div id="level-dist-body" style="display:flex; gap:10px; flex-wrap:wrap; align-items:center;"><!-- diisi JS --></div>
</div>
...
<div class="table-card">
  <div class="table-card-header">
    ...
    <span id="tbl-count">-- perusahaan</span>
  </div>
...
<div class="network-layout">
  ...
  <!-- Sidebar kiri -->
  <div class="net-sidebar" id="net-sidebar">
    <div class="net-sidebar-header">📋 Ringkasan Grup</div>
    <div class="net-sidebar-body" id="net-sidebar-body"><!-- diisi JS --></div>
  </div>
  ...
  <div class="net-canvas-wrap">
    <div class="net-top-bar">
      <button class="net-toggle-sidebar" onclick="toggleSidebar()">
        <span id="sidebar-icon">◀</span> <span id="sidebar-label">Sembunyikan</span>
      </button>
    ...
    <div class="net-filter-bar">
      <select id="net-sel-grup" onchange="renderNetwork()" style="min-width:200px;">
        <option value="ALL">— Semua Grup —</option>
      </select>
    </div>

    <div class="net-legend" id="net-legend">
...
<div class="modal-overlay" id="modal-overlay" onclick="closeModal(event)">
  <div class="modal" id="modal-box">
    <div class="modal-head">
      <div>
        <h2 id="modal-nama"></h2>
        <div class="npwp" id="modal-npwp"></div>
      </div>
...
// Isi dropdown grup
['sel-grup','net-sel-grup'].forEach(id => {{
...
function getGrupIds() {{
  const selGrup = document.getElementById('sel-grup').value;
...
function filterByGrup() {{
  ...
  const selStatus = document.getElementById('sel-status').value;
  const selTahun2 = document.getElementById('sel-tahun').value;
...
function updateKPI(ids, selStatus, selTahun2) {{
  ...
  document.getElementById('kpi-grup').textContent      = selGrup === 'ALL' ? grupList.length : 1;
  document.getElementById('kpi-perus').textContent     = ids.size;
  document.getElementById('kpi-terbit').textContent    = terbit;
  document.getElementById('kpi-belum').textContent     = belum;
  document.getElementById('kpi-potensi').textContent   = fmtRp(pot);
  document.getElementById('kpi-realisasi').textContent = fmtRp(real);
  document.getElementById('kpi-eff-sub').textContent   = 'Efektivitas: ' + eff + '%';
}}
...
function updateLevelDist(ids, selStatus, selTahun2) {{
  ...
  document.getElementById('level-dist-body').innerHTML = html || '<div class="empty-msg">Tidak ada data</div>';
}}
...
function renderTable(ids, selStatus, selTahun2) {{
  ...
  document.getElementById('tbl-count').textContent = ordered.length + ' perusahaan';
  ...
  document.getElementById('main-tbody').innerHTML = rows || '<tr><td colspan="12" class="empty-msg">Tidak ada data sesuai filter</td></tr>';
}}
...
function openModal(id) {{
  ...
  document.getElementById('modal-npwp').textContent = 'NPWP: ' + (p.npwp||'—') + '  |  ' + p.id;
  ...
  document.getElementById('modal-info-grid').innerHTML = igHtml;
  ...
  if (lhaFiltered.length === 0) {{
    document.getElementById('modal-lha-wrap').innerHTML =
    ...
    tHtml += '</tbody></table>';
    document.getElementById('modal-lha-wrap').innerHTML = tHtml;
  }}
...
function toggleSidebar() {{
  ...
  const sb = document.getElementById('net-sidebar');
  sb.classList.toggle('hidden', !sidebarVisible);
  document.getElementById('sidebar-icon').textContent  = sidebarVisible ? '◀' : '▶';
  document.getElementById('sidebar-label').textContent = sidebarVisible ? 'Sembunyikan' : 'Tampilkan';
...
function buildSidebar() {{
  ...
  document.getElementById('net-sidebar-body').innerHTML = html;
}}

function selectGrupNet(g) {{
  document.getElementById('net-sel-grup').value = g;
  // Highlight card
...
function buildLegend() {{
  ...
  document.getElementById('net-legend').innerHTML = html;
}}
...
function renderNetwork() {{
  const selGrup = document.getElementById('net-sel-grup').value;
...
(function init() {{
  ...
  document.getElementById('main-tbody').addEventListener('click', function(e) {{
    ...
    if (expBtn) {{
      ...
      const ss = document.getElementById('sel-status').value;
      const st = document.getElementById('sel-tahun').value;
  ...
  document.getElementById('net-sidebar-body').addEventListener('click', function(e) {{
...
class App(tk.Tk):
    def __init__(self):
        super().__init__()
        self.title("Generator Dashboard Grup Perusahaan — DJP")
        ...
        self._status = tk.StringVar(value="Pilih file Excel untuk memulai.")

        self._build_ui()

    def _build_ui(self):
        # ── Header ──────────────────────────────────────────
        header = tk.Frame(self, bg="#003566", height=70)
        header.pack(fill="x")
        header.pack_propagate(False)

        tk.Label(header, text="🏛", font=("Segoe UI Emoji", 22),
                 bg="#003566", fg="white").pack(side="left", padx=18)

        htext = tk.Frame(header, bg="#003566")
        htext.pack(side="left", pady=12)
        tk.Label(htext, text="Generator Dashboard Grup Perusahaan",
                 font=("Segoe UI", 13, "bold"),
                 bg="#003566", fg="white").pack(anchor="w")
        tk.Label(htext, text="Direktorat Jenderal Pajak  |  Analisa Data LHA Grup",
                 font=("Segoe UI", 9), bg="#003566", fg="#88BBEE").pack(anchor="w")

        # ── Body ─────────────────────────────────────────────
        body = tk.Frame(self, bg="#F0F7FF", padx=28, pady=20)
        body.pack(fill="both", expand=True)

        # File Excel
        self._section(body, "📂  File Sumber Excel", row=0)
        row1 = tk.Frame(body, bg="#F0F7FF")
        row1.grid(row=1, column=0, columnspan=2, sticky="ew", pady=(4, 14))
        body.columnconfigure(0, weight=1)

        entry1 = tk.Entry(row1, textvariable=self._excel_path,
                          font=("Segoe UI", 10), bg="white",
                          relief="solid", bd=1, fg="#0F172A")
        entry1.pack(side="left", fill="x", expand=True, ipady=6, padx=(0, 8))

        tk.Button(row1, text="Pilih File…",
                  font=("Segoe UI", 10, "bold"),
                  bg="#0052A5", fg="white", relief="flat",
                  activebackground="#003D7A", activeforeground="white",
                  padx=14, pady=6,
                  command=self._pick_excel).pack(side="left")

        # Output
        self._section(body, "💾  File Output HTML", row=2)
        row2 = tk.Frame(body, bg="#F0F7FF")
        row2.grid(row=3, column=0, columnspan=2, sticky="ew", pady=(4, 14))

        entry2 = tk.Entry(row2, textvariable=self._output_path,
                          font=("Segoe UI", 10), bg="white",
                          relief="solid", bd=1, fg="#0F172A")
        entry2.pack(side="left", fill="x", expand=True, ipady=6, padx=(0, 8))

        tk.Button(row2, text="Simpan ke…",
                  font=("Segoe UI", 10, "bold"),
                  bg="#0052A5", fg="white", relief="flat",
                  activebackground="#003D7A", activeforeground="white",
                  padx=14, pady=6,
        ...
        # Info box
        info_frame = tk.Frame(body, bg="#DBEEFF", bd=0, relief="flat")
        info_frame.grid(row=4, column=0, columnspan=2, sticky="ew", pady=(0, 16))
        tk.Label(info_frame,
                 text=(
                     "ℹ  Pastikan file Excel menggunakan format template DJP yang benar.\n"
                     "    Sheet yang diperlukan: Perusahaan, LHA.\n"
                     "    Output HTML bisa langsung dibuka di browser secara offline."
                 ),
                 font=("Segoe UI", 9), bg="#DBEEFF", fg="#003566",
                 justify="left", padx=12, pady=10).pack(anchor="w")

        # Generate button
        self._btn_gen = tk.Button(body,
            text="⚡  Generate Dashboard",
            font=("Segoe UI", 12, "bold"),
            bg="#0A7C4F", fg="white", relief="flat",
            activebackground="#065F3B", activeforeground="white",
            padx=20, pady=10,
            command=self._run_generate)
        self._btn_gen.grid(row=5, column=0, columnspan=2, pady=(0, 12))

        # Progress
        self._progress = ttk.Progressbar(body, mode="indeterminate", length=560)
        self._progress.grid(row=6, column=0, columnspan=2, sticky="ew", pady=(0, 8))

        # Status
        status_lbl = tk.Label(body, textvariable=self._status,
                               font=("Segoe UI", 9), bg="#F0F7FF",
                               fg="#475569", wraplength=560, justify="center")
        status_lbl.grid(row=7, column=0, columnspan=2)

    def _section(self, parent, text, row):
        tk.Label(parent, text=text,
                 font=("Segoe UI", 10, "bold"),
                 bg="#F0F7FF", fg="#003566").grid(row=row, column=0,
                 columnspan=2, sticky="w", pady=(6, 0))

    def _pick_excel(self):
        path = filedialog.askopenfilename(
            title="Pilih File Template Excel DJP",
        ...
        if path:
            ...
            self._output_path.set(base + "_dashboard.html")
            self._status.set("File dipilih: " + os.path.basename(path))

    def _pick_output(self):
        path = filedialog.asksaveasfilename(
            title="Simpan Dashboard HTML",
    ...
    def _run_generate(self):
        ...
        if not os.path.isfile(excel):
            ...
            return
        if not output:
            messagebox.showerror("Error", "Tentukan lokasi output HTML.")
            return

        self._btn_gen.config(state="disabled")
        self._progress.start(10)
        self._status.set("Memproses data... mohon tunggu.")

        def worker():
            try:
                p_rows, l_rows = read_excel(excel)
                perusahaan = build_data_model(p_rows, l_rows)
                generate_html(perusahaan, output, excel)
                self.after(0, self._on_success, output, len(perusahaan))
            except Exception as ex:
                import traceback
                tb = traceback.format_exc()
                self.after(0, self._on_error, str(ex), tb)

        threading.Thread(target=worker, daemon=True).start()

    def _on_success(self, output, n):
        self._progress.stop()
        self._btn_gen.config(state="normal")
        self._status.set("✅ Dashboard berhasil dibuat! " + str(n) + " perusahaan diproses.")
        msg = (
            "Dashboard HTML berhasil dibuat!\n\n"
            "File: " + output + "\n"
            "Perusahaan: " + str(n) + "\n\n"
            "Buka file tersebut di browser untuk melihat dashboard."
        )
        if messagebox.askyesno("Sukses!", msg + "\n\nBuka sekarang di browser?"):
            import webbrowser
    ...
    def _on_error(self, msg, tb):
        ...
        messagebox.showerror("Error saat generate",
                             "Terjadi kesalahan:\n\n" + msg + "\n\nDetail:\n" + tb[:600])
